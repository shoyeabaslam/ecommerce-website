...
const OrdersView = ({orderItems,setIsOpen,productImages,toast}) => {
  ...
  const saveChanges = async () =>{
    setIsLoading(true)
    try {
      const orderData = {id:orderItems._id,orderStatus:selectedStatus}
      const response = await axios.post("/api/updateOrders", {
        data: orderData,
      });
      setIsLoading(false)
  ...
  return (
    <div className="bg-yellow-100 fixed h-[100vh] overflow-auto top-[65px] left-[5%] right-[5%] flex flex-col py-10 px-8">
      ...
      {isLoading && (
        ...
        <div className="pb-10 flex justify-center"><button onClick={saveChanges} className="bg-lightred shadow-shadow2 rounded-lg px-2 py-1 text-white">Save Changes</button></div>
      </div>
    </div>
  );
};

const TotalOrder = ({ torders ,productImages,toast}) => {
  const statusColor = { "Pending": "bg-orange-300", Delivered: "bg-green-300", OutOfDelivery:'bg-blue-400',Processing:'bg-rose-400'};
    ...
    <div className="p-8 font-Roboto relative">
      ...
      <div>
       ...
       <input
        ...
        />
        <div className="flex items-center ml-10 "><div className="bg-orange-300 rounded-md w-8 h-8 mr-1"/>Pending</div>
        <div className="flex items-center ml-10 "><div className="bg-rose-400 rounded-md w-8 h-8 mr-1"/>Processing</div>
        <div className="flex items-center ml-10 "><div className="bg-blue-400 rounded-md w-8 h-8 mr-1"/>OutOfDelivery</div>
       </div>
